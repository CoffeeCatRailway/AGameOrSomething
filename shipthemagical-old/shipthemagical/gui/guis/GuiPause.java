...
import io.github.coffeecatrailway.shipthemagical.gui.components.sheetwords.Resume;
...
public class GuiPause extends Gui {
	...
	/**
	 * GuiPause.java constructor.
	 * 
	 * @param window {@code Window}
	 */
	public GuiPause(Window window) {
		super(window, "gui_pause");
		...
		resume = new Button(resume_x + x_offset, resume_y + y_offset, button_width, button_height);

		float resume_word_width = 20;
		float resume_word_height = 32;
		float resume_word_x = (-button_width / 2) + 14;
		float resume_word_y = (-(resume_word_height * 2) - resume_word_height) + 4;
		resume_word = new Resume(resume_word_x + x_offset, resume_word_y + y_offset, resume_word_width,
				resume_word_height);
	...
	public void update(Input input) {
		...
		if (input.isKeyDown(GLFW_KEY_ESCAPE) && !render) {
	...
	public void render() {
		...
		if (render) {
			...
			resume.render(camera, sheet, shader);
			resume_word.render(camera, sheet, shader);
